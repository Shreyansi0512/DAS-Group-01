...
p1<-ggplot(DataSet, aes(x = Total.Household.Income, y = Total.Number.of.Family.members)) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("Total Household Income vs. Number of Family Members")


p2<-ggplot(DataSet, aes(x = Total.Food.Expenditure, y = Total.Number.of.Family.members)) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("Total Food Expenditure vs. Number of Family Members")

p3<-ggplot(DataSet, aes(x = Household.Head.Age, y = Total.Number.of.Family.members)) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("Household Head Age vs. Number of Family Members")

p4<-ggplot(DataSet, aes(x = House.Floor.Area, y = Total.Number.of.Family.members)) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("House Floor Area vs. Number of Family Members")

p5<-ggplot(DataSet, aes(x = House.Age, y = Total.Number.of.Family.members)) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("House Age vs. Number of Family Members")

p6<-ggplot(DataSet, aes(x = Number.of.bedrooms, y = Total.Number.of.Family.members)) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("Number of Bedrooms vs. Number of Family Members")

p7<-ggplot(data = DataSet, aes(x = Type.of.Household, y = Total.Number.of.Family.members)) +
  geom_boxplot() +
  ggtitle("Type of Household vs. Number of Family Members")

p8<-ggplot(data = DataSet, aes(x = Total.Food.Expenditure, y = Total.Number.of.Family.members, group = Electricity)) +
  geom_point() +
  geom_smooth(method = "lm", se=FALSE,aes(color = Electricity)) +
...
## Model 1: Logistic Regression Binomial Distribution {#sec:sub}
```{r}
dataset <- read.csv("~/Desktop/Project 2/dataset1.csv")
...
factor_variables <- c("Type.of.Household", "Electricity", "Region")
for (var in factor_variables) {
  cat(var, "has", nlevels(dataset[[var]]), "levels\n")
}

dataset$FamilySize_AboveMedian <- ifelse(dataset$Total.Number.of.Family.members > median(dataset$Total.Number.of.Family.members, na.rm = TRUE), 1, 0)

model <- glm(FamilySize_AboveMedian ~ Total.Household.Income +
...
We then to evaluate the performance of this logistic regression model; the AUC-ROC is commonly utilized to evaluate the effectiveness of a binary classification model, with a value of 1 denoting perfect classification and 0.5 suggesting a random guessing. By plotting the true positive rate against the false positive rate for differing predicted probability thresholds, the ROC curve provides a visual representation illustrating the sensitivity-versus-specificity trade-offs of the model for various points.
Overall, evaluating the AUC-ROC value allows us to determine how adequate the model performs for its intended purpose. A higher value suggests that the model is proficient in distinguishing between the two classes.
```{r}
set.seed(42)
train_index <- createDataPartition(dataset$FamilySize_AboveMedian, p = 0.7, list = FALSE)
train_set <- dataset[train_index, ]
test_set <- dataset[-train_index, ]
model <- glm(formula = FamilySize_AboveMedian ~ Total.Household.Income +
               Total.Food.Expenditure + Household.Head.Age + Type.of.Household +House.Age, family = binomial(link = "logit"), data = train_set)
predictions_prob <- predict(model, newdata = test_set, type = "response")
roc_obj <- roc(test_set$FamilySize_AboveMedian, predictions_prob)
auc(roc_obj)
plot(roc_obj, main = "ROC curve")
```
The ROC value is 0.7727 higher than 0.5 close to 1 indicates a good performance of the model. 
The plot below shows the predicted probabilities for different combinations of predictor variable values. It illustrate relationship between the predictor variables and the predicted probabilities of the response variable FamilySize_AboveMedian being equal to 1 (the probability of family size above median). The y-axis of the plot represents the predicted probability of the response variable being equal to 1, and the x-axis of the plot represents the values of the predictor variables.
```{r, eval = TRUE, out.width = '80%', fig.align = "center", fig.cap = "\\label{fig:Probability}  Plot predicted probabilities of predictor variable values", fig.pos = "H"}
# Calculate and display confidence intervals for the model coefficients
confint(improved_model) %>%
  kable()
mod.coef.logodds <- summary(improved_model) %>%
  coef()
plot_model(model, type = "pred", title = "",
           axis.title = c("Predictor Variables", "Probability of FamilySize_AboveMedian"))
```
...
```{r, eval = TRUE, out.width = '80%', fig.align = "center", fig.cap = "\\label{fig:Probability}  Log-Odds plot", fig.pos = "H"}
dataset <- read_csv("dataset1.csv")
dataset$logodds_FamilyMembers <- predict(improved_model, type = "link")

ggplot(dataset, aes(x = Total.Household.Income, y = logodds_FamilyMembers)) +
  geom_smooth(method = "loess") +
  labs(x = "Total Household Income", y = "Probability of FamilySize_AboveMedian") +
  scale_y_continuous(labels = function(x) exp(x), trans = "log")

# Add a line for the null model
null_model <- glm(Total.Number.of.Family.members ~ 1, data = dataset, family = "poisson")
null_logodds <- predict(null_model, type = "link")
# Create a log-odds plot 
ggplot(dataset, aes(x = Total.Household.Income, y = logodds_FamilyMembers)) +
  geom_smooth(method = "loess") +
  labs(x = "Total Household Income", y = "Probability of FamilySize_AboveMedian") +
  scale_y_continuous(labels = function(x) exp(x), trans = "log") +
  geom_hline(yintercept = null_logodds, lty = 2, col = "gray")

```

## Model 2: Logistic regression Poission Distribution {#sec:sub}
...
### Importance ranking of variables {#sec:sub}
```{R include=FALSE}
library(relaimpo);
```
```{R}
...
# Conclusion {sec:Conc}
From the results we get above, we have concluded that Total.Household.Income and Total.Food.Expenditure are the two most important variables. However, the average coefficients are still relatively small. This can be attributed to several reasons. The most significant reason is that Income and Expenditure are not on the same scale as the number of family members.
Actually, we can see from the output above that roughly each additional unit of income, the number of household members increase by 2.276022e-06, namely every additional 407947 units of household income could attribute to one member plus. So high income families tend to have more members.
As for the food expenditure, it does not against our common sense that with the number of family members increase, the food expenditure would rise with a 2.539690e-05 coefficient and relatively high correlation.
